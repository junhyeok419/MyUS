	...
	m_Opacity(1.f)
{
	...
	m_CBuffer->SetFullColor(m_FullColor);
...
bool CMaterial::GetOnlyColor()
{
	return m_OnlyColor;
}

void CMaterial::SetOnlyColor(bool Switch)
{
	m_OnlyColor = Switch;

	m_CBuffer->SetOnlyColor(m_OnlyColor);
}

void CMaterial::SetFullColor(const Vector4& Color)
{
	m_FullColor = Color;
...
void CMaterial::SetFullColor(float r, float g, float b, float a)
{
	m_FullColor = Vector4(r, g, b, a);

	m_CBuffer->SetFullColor(m_FullColor);
}

void CMaterial::SetFullColorUnsignedChar(unsigned char r, unsigned char g, unsigned char b, unsigned char a)
{
	m_FullColor = Vector4(r / 255.f, g / 255.f, b / 255.f, a / 255.f);
...
void CMaterial::Save(FILE* File)
{
	...
	fwrite(&m_FullColor, sizeof(Vector4), 1, File);
...
void CMaterial::Load(FILE* File)
{
	...
	fread(&m_FullColor, sizeof(Vector4), 1, File);
	...
	m_CBuffer->SetBaseColor(m_FullColor);
	m_CBuffer->SetBaseColor(m_BaseColor);
	m_CBuffer->SetAmbientColor(m_AmbientColor);
	m_CBuffer->SetSpecularColor(m_SpecularColor);
	m_CBuffer->SetEmissiveColor(m_EmissiveColor);
